...
export class SpeechController {
  constructor(
    private voicebox: VoiceboxService,
    private fragmentSetService: FragmentsetService,
  ) {}
  /**
   * tts
   */
  ...
  async tts(@Body() postParams): Promise<any> {
    const message = postParams['text'] ?? '';
    const tts = await this.voicebox.tts(message);
  ...
  async download(@Param() params): Promise<any> {
    ...
    const fragmentSet = await this.fragmentSetService.byHash(params.hash ?? '');

    if (fragmentSet) {
      const file = await this.voicebox.getAudio(fragmentSet, params.format);
    } else {
