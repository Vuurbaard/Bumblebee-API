...
export class AppController implements RESTController {
  ...
  public async getByID(req: Request, res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Something went wrong getting an app by id." });
    }
  }
  ...
  public async updateByID(req: Request, res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Something went wrong updating an app by id." });
    }
  }
  ...
  public async create(req: Request, res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Something went wrong creating the app." });
    }
  }
  ...
  public async deleteByID(req: Request, res: Response) {
    try {
      if (this.isAdmin(req)) {
        ...
        const app = await appService.getOne({
          _id: req.params.id,
          createdBy: req.user.id,
        });
        if (app) {
      ...
      res
        .status(500)
        .json({ message: "Something went wrong deleting the app." });
    }
  }
}
