    ...
    try {
      ...
      if (username && password) {
              ...
              (err: any, success: boolean) => {
                if (err) {
                  ...
                  res
                    ...
                    .json({
                      message:
                        "The supplied username/password combination is wrong.",
                    });
                } else {
                  ...
                  res.json({
                    token:
                      "JWT " +
                      jwt.sign(userObj, "SomethingVerySecret", {
                        expiresIn: "100 years",
                      }),
                    user: userObj,
                  });
        ...
        App.findOne({ token: token }, (err: any, app: IApp) => {
          ...
          if (!app) {
            res.status(401).json({ message: "The supplied token is invalid." });
          } else {
            res.json({
              token:
                "JWT " +
                jwt.sign(app, "SomethingVerySecret", {
                  expiresIn: "100 years",
                }),
              app: app,
            });
        ...
        res
          ...
          .json({
            message:
              "You need to supply a username/password combination or a token in the request.",
          });
      }
    } catch (err) {
