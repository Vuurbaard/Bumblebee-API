...
export class UserController implements RESTController {
  ...
  public async getAll(req: Request, res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Something went wrong getting all the users." });
    }
  }
  ...
  public async getByID(req: Request, res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Something went wrong getting a user by id." });
    }
  }
    ...
    try {
      if (req.user!.roles.indexOf("admin") > -1) {
        ...
        if (req.user!._id.toString() != req.params.id) {
          throw new Error("Not allowed to update another user.");
        }
        if (req.body.username) {
          throw new Error("Not allowed to update your own username.");
        }
        if (req.body.password) {
          throw new Error("Not allowed to update your own password.");
        }
        if (req.body.roles) {
          throw new Error("Not allowed to update your own roles.");
        }
      ...
      if (
        err.message == "Not allowed to update another user." ||
  ...
  public async create(req: Request, res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Something went wrong creating the new user." });
    }
  }
  ...
  public async deleteByID(req: Request, res: Response) {
    try {
      if (req.user!.roles.indexOf("admin") > -1) {
        ...
        res
          .status(403)
          .json({ message: "You are not allowed to delete a user" });
        return;
      }
    } catch (err) {
      console.error(err.message);
      res
        .status(500)
        .json({ message: "Something went wrong deleting the user." });
    }
  }
