...
export class SourceController implements RESTController {
  ...
  public async getAll(req: Request, res: Response) {
    if (await cacheService.hasKey("all-fragments")) {
      ...
      try {
        const sources = (await sourceService.all(req.query)).map(function (
          item
        ) {
          return {
            id: item.id,
        ...
        res
          .status(500)
          .json({ message: "Something went wrong getting all the sources." });
      }
    }
  }
  ...
  public async getByID(req: Request, res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Something went wrong getting source by id." });
    }
  }
